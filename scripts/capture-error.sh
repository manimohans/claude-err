...
if [ "$HOOK_EVENT" = "PostToolUseFailure" ]; then
  # error field has the failure message; also try tool_response for full output
...
${EXTRA}"
  ...
  TOOL_RESPONSE=$(echo "$INPUT" | jq -r 'if .tool_response | type == "string" then .tool_response else (.tool_response | tostring) end' 2>/dev/null || true)
